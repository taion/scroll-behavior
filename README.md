...
It can sometimes be convenient to know in the `shouldUpdateScroll` function whether there is a saved scroll state that will be restored. You can obtain it by calling `this.getSavedWindowScrollTarget()` for `window` or `this.getSavedElementScrollTarget(key)` for a specific element (see below).
